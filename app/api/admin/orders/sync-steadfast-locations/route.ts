...
export async function GET(request: NextRequest) {
  try {
    ...
    // Call Steadfast get_police_stations API
    const res = await fetch("https://portal.steadfast.com.bd/api/v1/get_police_stations", {
      method: "GET",
      headers: {
        "Api-Key": apiKey,
    ...
    if (res.status === 200 && body.status === 200 && Array.isArray(body.data)) {
      ...
      const metroThanas = [
        'Demra', 'Dhaka Cantt.', 'Dhanmondi', 'Gulshan', 'Jatrabari',
