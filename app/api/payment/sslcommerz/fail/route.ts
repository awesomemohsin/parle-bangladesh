...
export async function POST(req: Request) {
  ...
  if (tran_id) {
    return NextResponse.redirect(new URL(`/shop/order-received/${tran_id}?payment=failed`, appUrl), 303);
  }
  return NextResponse.redirect(new URL("/orders?payment_status=failed", appUrl), 303);
}
