...
export default function ApprovalDashboardPage() {
  const [stats, setStats] = useState({ products: 0, orders: 0, promos: 0, customers: 0 })

  useEffect(() => {
    const fetchStats = async () => {
      try {
        const [prodRes, orderRes, promoRes, custRes] = await Promise.all([
          ...
          fetch('/api/approvals?type=customer&status=pending', { headers: { Authorization: `Bearer ${localStorage.getItem('token')}` } })
        ])
        ...
        setStats({ 
          ...
          customers: custData.requests?.length || 0
        })
      } catch (e) {}
    }
    fetchStats()
  ...
  return (
    <div className="space-y-10 pb-20">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
        ...
        <Link href="/admin/approvals/customers">
           <Card className="p-8 border-2 border-gray-100 hover:border-black transition-all group overflow-hidden relative h-full">
              <div className="absolute top-0 right-0 w-24 h-24 bg-indigo-600/5 rounded-full -translate-y-12 translate-x-12 scale-150 transition-transform group-hover:scale-[2] duration-500"></div>
              <div className="space-y-4 relative z-10">
                 <div className="flex justify-between items-start">
                    <span className="text-[10px] font-black uppercase text-indigo-600 bg-indigo-50 px-2 py-1 rounded tracking-widest">Customer Level</span>
                    <span className="text-4xl font-black text-gray-900">{stats.customers}</span>
                 </div>
                 <div>
                    <h2 className="text-2xl font-black text-gray-900 uppercase">Promotions</h2>
                    <p className="text-gray-400 text-xs font-medium leading-relaxed italic">Customer promotion requests require Level 2 verification to be activated.</p>
                 </div>
                 <div className="flex items-center gap-2 text-gray-300 group-hover:text-black transition-colors font-black text-[9px] uppercase tracking-widest pt-4">
                    Enter Verification Terminal →
                 </div>
